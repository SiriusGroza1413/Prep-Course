...
function agregarItemAlComienzoDelArray(array, elemento) {
  ...
  return array;
} // var array = [1, 2, 3, 4, 5] for(var = i; i < array.length; i++)
  //              0  1  2  3  4 
...
function cuentoElementos(arreglo){
  ...
  for(var i = 0; i < arreglo.length; i++) {
    if(arreglo[i] > 19) contadorDeVueltas ++
  }
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  var convertidoAString = n.toString();
  if(convertidoAString[0] === "9") {
    return true
  }
return false
} 
...
function breakStatement(numero) {
  ...
  // Tu código:
  var arrayConNuevoValor = []
  for(var i = 0; i < 10; i++){
    numero = numero + 2;
    if(numero === i){
      break
    }else{
      arrayConNuevoValor.push(numero)
    }
  }
  if(arrayConNuevoValor.length < 10){
    return "Se interrumpió la ejecución"
  } else {
    return arrayConNuevoValor;
  }
}
